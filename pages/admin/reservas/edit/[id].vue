...
import { getReservation, updateReservation, isAuthenticated } from '@/services/api'
...
export default {
  setup() {
    ...
    const form = ref({
      ...
      payment_intent_id: '' // Añadido para mantener este campo
    })
    ...
    const fetchReservation = async () => {
      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          console.log("Reserva cargada (array):", reservationData)
        } else if (typeof data === 'object' && data !== null) {
          reservationData = data
          console.log("Reserva cargada (objeto):", reservationData)
        } else {
          throw new Error('Formato de respuesta inesperado')
        }
        
        originalReservation.value = reservationData
        
        // Llenar el formulario con los datos de la reserva
        form.value.name = reservationData.name || ''
        form.value.surname = reservationData.surname || ''
        form.value.email = reservationData.email || ''
        form.value.phone = reservationData.phone || ''
        form.value.language = reservationData.language || 'es'
        form.value.people_count = reservationData.people_count || 1
        ...
        form.value.time = reservationData.time || '10:00:00'
        form.value.allergies = reservationData.allergies || ''
        form.value.payment_method = reservationData.payment_method || 'Tarjeta'
        form.value.payment_status = reservationData.payment_status || 'succeeded'
        form.value.payment_intent_id = reservationData.payment_intent_id || ''
        ...
        try {
          ...
          if (reservationData.brunches) {
            let brunches
            
            if (typeof reservationData.brunches === 'object') {
              brunches = reservationData.brunches
            } else if (typeof reservationData.brunches === 'string') {
              brunches = JSON.parse(reservationData.brunches)
            }
            
            if (brunches) {
              Object.values(brunches).forEach(brunch => {
                if (brunch.title === 'Metropolitan') {
                  form.value.brunches.metropolitan = parseInt(brunch.quantity) || 0
                } else if (brunch.title === 'Deluxe') {
                  form.value.brunches.deluxe = parseInt(brunch.quantity) || 0
                } else if (brunch.title === 'New York') {
                  form.value.brunches.newYork = parseInt(brunch.quantity) || 0
                }
              })
            }
          }
    ...
    const totalBrunchs = computed(() => {
      return parseInt(form.value.brunches.metropolitan || 0) + 
             parseInt(form.value.brunches.deluxe || 0) + 
             parseInt(form.value.brunches.newYork || 0)
    })
    ...
    const saveReservation = async () => {
      ...
      try {
        ...
        const reservationData = {
          ...
          payment_intent_id: form.value.payment_intent_id || originalReservation.value.payment_intent_id || '',
